...
//            String daxPath = "data/dax/Sipht_1000.xml";
        ...
        for (Job job : cloudletReceivedList) {
            for (Job parent : job.getParentList()) {
                if (job.getExecStartTime() < parent.getExecFinishTime()) {
                    StaticLog.warn("job " + job.getName() + " start time " + job.getExecStartTime() + " < parent " + parent.getName() + " finish time " + parent.getExecFinishTime());
                    break;
                }
            }
            for (Job child : job.getChildList()) {
                if (job.getExecFinishTime() > child.getExecStartTime()) {
                    StaticLog.error("job " + job.getName() + " finish time " + job.getExecFinishTime() + " > child " + child.getName() + " start time " + child.getExecStartTime());
                    break;
                }
            }
        }
