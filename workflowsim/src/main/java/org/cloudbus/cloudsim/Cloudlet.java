...
public class Cloudlet {
    ...
    public void finalizeCloudlet() {
        ...
        if (getStatus() == CloudletStatus.SUCCESS) {
    ...
    public boolean updateStatus(final CloudletStatus status) {
        ...
        CloudletStatus prevStatus = getStatus();
        ...
        if (prevStatus == CloudletStatus.INEXEC) {
            // and current status is either CANCELED, PAUSED or SUCCESS
            if (status == CloudletStatus.CANCELED || status == CloudletStatus.PAUSED || status == CloudletStatus.SUCCESS) {
        ...
        if (prevStatus == CloudletStatus.RESUMED && status == CloudletStatus.SUCCESS) {
            // then update the Cloudlet completion time
            totalCompletionTime += (clock - execStartTime);
            return true;
        }
        ...
        if (status == CloudletStatus.INEXEC || (prevStatus == CloudletStatus.PAUSED && status == CloudletStatus.RESUMED)) {
